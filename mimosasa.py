...
helpmenuEN = {
    ...
    "1": textwrap.wrap("View individual %ss in detail, create filtered overviews, analyze your income and expenses. To start, the daily balance is displayed, along with the monthly score per %s." % (woordtransactieEN,woordcategorieEN),w),
        ...
        "1,4": textwrap.wrap("Here you can see a compact overview of your score per month, and how it compares to the rest of the year (shown in the last two columns). The %s budget should approximately match the MonthlyAverage. You can also use this report to establish the budgets (retroactively). The month you want to view the report for is entered as one or two digits." % (woordcategorieEN),w),
...
helpmenuIT = {
    ...
    "1": textwrap.wrap("Visualizza le singole Transazioni in dettaglio, crea riepiloghi filtrati, analizza i tuoi guadagni e le tue spese. Per iniziare verrà mostrato il saldo giornaliero e il punteggio mensile per %s." % (woordcategorieIT),w),
        ...
        "1,4": textwrap.wrap("Guarda qui in un riepilogo compatto quale è il tuo punteggio mensile e come si confronta con il resto dell'anno (mostrato nelle ultime due colonne). Il budget per %s deve corrispondere approssimativamente alla MediaMensile. Puoi quindi utilizzare questo rapporto anche per stabilire i budget (con effetto retroattivo). Il mese di cui desideri visualizzare il rapporto viene inserito come uno o due cifre." % (woordcategorieIT),w),
...
helpmenu = {
    ...
    "1": textwrap.wrap("Toon individuele %ss in detail, maak gefilterde samenvattingen, analyseer je inkomsten en uitgaven. Als eerste wordt het dagsaldo en de maandelijkse score per categorie getoond." % (woordtransactie),w),
        ...
        "1,4": textwrap.wrap("Bekijk hier in een compact overzicht wat je score per maand is, en hoe die zich verhoudt tot de rest van het jaar (getoond in de laatste twee kolommen). Het %sbudget dient ongeveer overeen te komen met het MaandelijksGemiddelde. Je kunt deze rapportage daarom ook gebruiken om (met terugwerkende kracht) de budgetten vast te stellen. De maand waarvan je de rapportage wilt bekijken wordt als één of twee cijfers ingegeven." % (woordcategorie),w),
...
def printheaderall(rekening): # geen H
    ...
    print(kleuren["LichtRood"]+("{:<%d}" % (maxlen)).format(menulinks[12])+kleuren["Blauw"]+": "+kleuren["LichtGroen"]+str(header[nieuwheaderlijst[12]])+ResetAll)
    ...
    print(kleuren["LichtRood"]+("{:<%d}" % (maxlen)).format(menulinks[15])+kleuren["Blauw"]+": "+kleuren["LichtGroen"]+vertaalv(header[nieuwheaderlijst[15]])+ResetAll)
    print(kleuren["LichtRood"]+("{:<%d}" % (maxlen)).format(menulinks[16])+kleuren["Blauw"]+": "+kleuren["LichtGroen"]+vertaalv(header[nieuwheaderlijst[16]])+ResetAll)
...
def geefeenbedrag(rekening,header,col,ok,bedrag): # H
    ...
    if bedrag == standaardbodembedrag:
        ...
        while loop == True:
            ...
            if header["Autodecimalen"] in jalijst:
                bedragkeuze = str(int(bedragkeuze)/100)
...
def maandanalyse(rekening,datumlijst,ok): # H
    kleuren,catcol = updatekleuren(rekening)
    header = haalheader(rekening)
    Taal = header[nieuwheaderlijst[3]]
    valuta = header[nieuwheaderlijst[4]]
    ...
    for i in categorieenlijst:
        with open(os.path.join(rekening,i),"r") as cat:
            ...
            if ca[0][1] < 0:
                ...
                c = [i+"yay",ca[0][1]]
                for j in ca[1:]:
                    if int(datumlijst[0]) <= j[0] <= int(datumlijst[1]):
                        caant += 1
                        catot += j[1]
                        yay += j[1]
                c.append(round(catot,2))
                c.append(caant)
                yaylijst.append(c)
            else: 
                caant = 0
    ...
    if yay < budyay * -1:
        yaya = budyay * -1
        binnen = kleuren["colslecht"]+"-"+kleuren["ResetAll"]
        binnenmc = "-"
    else:
        yaya = yay
        binnen = kleuren["colgoed"]+"+"+kleuren["ResetAll"]
        binnenmc = "+"
    voortlen = 43
    analijndatum = col+"+"+"%s-%s" % (datumlijst[0],datumlijst[1])+"+"+"<0%"+"-"*(int((voortlen-1)/2)-3)+"|"+"-"*(int((voortlen-1)/2)-5)+"100%>"+"+--%--+"+"---"+valuta+"----+-+"+kleuren["ResetAll"]
    analijndatummc = "+"+"%s-%s" % (datumlijst[0],datumlijst[1])+"+"+"<0%"+"-"*(int((voortlen-1)/2)-3)+"|"+"-"*(int((voortlen-1)/2)-5)+"100%>"+"+--%--+"+"---"+valuta+"----+-+"
    analijn =  col+"+-+"+"-"*15+"+"+"-"*int((voortlen-1)/2)+"|"+"-"*int((voortlen-1)/2)+"+-----+--------+-+"+kleuren["ResetAll"]
    analijnmc =  "+-+"+"-"*15+"+"+"-"*int((voortlen-1)/2)+"|"+"-"*int((voortlen-1)/2)+"+-----+--------+-+"
    totalijn = col+"+"+"-"*19+"+"+"-"*19+"+"+"-"*19+"+"+"-"*18+"+"+kleuren["ResetAll"]
    totalijnmc = "+"+"-"*19+"+"+"-"*19+"+"+"-"*19+"+"+"-"*18+"+"
    print()
    print(analijndatum)
    if header[nieuwheaderlijst[13]] == ">":
        with open(os.path.join(rekening,str(datumlijst[0])[:6])+"a","w") as a:
            print(analijndatummc, file = a)
    if Taal == "EN":
        uin = " In:"
        uit = " Out:"
        Tot = " TOT:"
        ...
        Tot = " TOT:"
        BeD = " Betaaldag:"
    uinlijn = col+"+"+kleuren["ResetAll"]+kleuren["colgoed"]+uin+kleuren["ResetAll"]+" "*(80-2-len(uin))+col+"+"+kleuren["ResetAll"]
    uinlijnmc = "+"+uin+" "*(80-2-len(uin))+"+"
    print(uinlijn)
    if header[nieuwheaderlijst[13]] == ">":
    ...
    for i in yaylijst:
        ...
        calen = len(str(i[3]))                      # de lengte in karakters van dat getal
        if 0 < voort < voortlen:
            voorts = voort
            colvoort = kleuren["colslecht"]
            streepje = "<"
            buffer = 0
        elif voort > voortlen:
            voorts = voortlen
            colvoort = kleuren["colgoed"]
            streepje = ">"
            buffer = 0
        elif voort == 0:
            voorts = voort
            colvoort = kleuren["colonbepaald"]
            streepje = "="
            buffer = -1
        elif 0 > voort > voortlen*-1: 
            voorts = voort
            colvoort = kleuren["colgoed"]
            streepje = ">"
            buffer = 0
    ...
    if header[nieuwheaderlijst[13]] == ">":
        with open(os.path.join(rekening,str(datumlijst[0])[:6])+"a","a") as a:
            print(analijnmc, file = a)
    print(totalijn)
    if header[nieuwheaderlijst[13]] == ">":
        with open(os.path.join(rekening,str(datumlijst[0])[:6])+"a","a") as a:
            print(totalijnmc, file = a)
    totlijn = col+"|"+kleuren["ResetAll"]+uit+" "+grotegetalkleuren(rekening,header,cry)+valuta+forr8(str(grootgetal(cry,fornum,K)[0])+grootgetal(cry,fornum,K)[2])+kleuren["ResetAll"]+" "*(19-len(uit)-9)+uin+" "+grotegetalkleuren(rekening,header,yay)+valuta+forr8(str(grootgetal(yay,fornum,K)[0])+grootgetal(yay,fornum,K)[2])+kleuren["ResetAll"]+" "*(19-len(uin)-9-1)+col+"|"+kleuren["ResetAll"]+Tot+" "+kleuren["Omkeren"]+grotegetalkleuren(rekening,header,yaya+cry)+valuta+forr8(str(grootgetal(yaya+cry,fornum,K)[0])+grootgetal(yaya+cry,fornum,K)[2])+kleuren["ResetAll"]+" "*(19-len(Tot)-9)+BeD+" "+binnen+" "*(19-len(BeD)-3)+col+"|"+kleuren["ResetAll"]
    totlijnmc = "|"+uit+" "+valuta+forr8(str(grootgetal(cry,fornum,K)[0])+grootgetal(cry,fornum,K)[2])+" "*(19-len(uit)-9)+uin+" "+valuta+forr8(str(grootgetal(yay,fornum,K)[0])+grootgetal(yay,fornum,K)[2])+" "*(19-len(uin)-9-1)+"|"+Tot+" "+valuta+forr8(str(grootgetal(yaya+cry,fornum,K)[0])+grootgetal(yaya+cry,fornum,K)[2])+" "*(19-len(Tot)-9)+BeD+" "+binnenmc+" "*(19-len(BeD)-3)+"|"
    print(totlijn)
    if header[nieuwheaderlijst[13]] == ">":
        with open(os.path.join(rekening,str(datumlijst[0])[:6])+"a","a") as a:
            print(totlijnmc, file = a)
    print(totalijn)
    if header[nieuwheaderlijst[13]] == ">":
        with open(os.path.join(rekening,str(datumlijst[0])[:6])+"a","a") as a:
            print(totalijnmc, file = a)
    ...
    while loop == True:
        ...
        if nieuwetransactieinputlijst[0].upper() in afsluitlijst:
            ...
            if nieuwetransactieinputlijst[0] == "":
                ...
                if test == True:
                    if len(str(nieuwetransactieinputlijst[0])) == 8:
                        test = checkdatum(nieuwetransactieinputlijst[0])
                        if test == True:
            ...
            if checkfloat(nieuwetransactieinputlijst[1]) == True:
                if header["Autodecimalen"] in jalijst:
                    nieuwetransactieinputlijst[1] = int(nieuwetransactieinputlijst[1])/100
...
def nieuwkopie(rekening,header,col,ok): # geen H
    ...
    for i in ok:
        ...
        if Taal == "EN":
            ...
            print(kleuren["Omkeren"]+col+"Nuova copia:"+kleuren["ResetAll"])
        elif Taal == "CJ":
            print(kleuren["Omkeren"]+col+"huwaxi hoziqi:"+kleuren["ResetAll"])
        else:
            print(kleuren["Omkeren"]+col+"Nieuwe kopie:"+kleuren["ResetAll"])
        dezeok = toontransactie(rekening,header,col,dezeok)
...
def wijzigautodecimalen(rekening,header): # geen H
    kleuren,catcol = updatekleuren(rekening)
    Taal = header[nieuwheaderlijst[3]]
    IBAN = rekening[:rekening.index("#")]
    JAAR = forr4(rekening[rekening.index("#")+1:])
    if Taal == "EN":
        vraag = textwrap.wrap("Change %s %s %s from %s to:" % (IBAN,JAAR,nieuwheaderlijstEN[16],coljanee(rekening,header,header[nieuwheaderlijst[16]])+vertaalv(header[nieuwheaderlijst[16]])+kleuren["ResetAll"]),w)
    elif Taal == "IT":
        vraag = textwrap.wrap("Cambia %s %s %s da %s in:" % (IBAN,JAAR,nieuwheaderlijstIT[16],coljanee(rekening,header,header[nieuwheaderlijst[16]])+vertaalv(header[nieuwheaderlijst[16]])+kleuren["ResetAll"]),w)
    elif Taal == "CJ":
        vraag = textwrap.wrap("hazüi %s %s %s qi %s qe:" % (IBAN,JAAR,nieuwheaderlijstCJ[16],coljanee(rekening,header,header[nieuwheaderlijst[16]])+vertaalv(header[nieuwheaderlijst[16]])+kleuren["ResetAll"]),w)
    else:
        vraag = textwrap.wrap("Wijzig %s %s %s van %s naar:" % (IBAN,JAAR,nieuwheaderlijst[16],coljanee(rekening,header,header[nieuwheaderlijst[16]])+vertaalv(header[nieuwheaderlijst[16]])+kleuren["ResetAll"]),w)
    for i in vraag:
        print(i)
    antwoord = geefjaofnee(rekening,header)
    if antwoord.upper() in afsluitlijst:
        ...
        header[nieuwheaderlijst[16]] = antwoord
        with open(os.path.join(rekening,"header"),"w") as h:
            print(header, file = h, end = "")
    return header
